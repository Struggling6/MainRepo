...
This script now delegates preprocessing, temporal windowing, scaling,
undersampling, and oversampling to LeadCSVHandler.

The script itself only:
  - forces local mode
  - disables loading precomputed artifacts while generating them
  - calls handler.get_dataloaders(...)
  - saves the resulting tensors as .npz files

Example:
    python scripts/precompute_lead_windows.py --num-clients 5 --overwrite
...
def main() -> None:
    ...
    # Precompute is only for already split local files:
    # datasets/LEAD/data1.csv, data2.csv, ...
    config.federation.partition_mode = "local"

    # Important:
    # During generation, do not try to load old precomputed artifacts.
    ...
    for partition_id in range(num_clients):
        ...
        # In the new design, this is "after all balancing", because the handler
        # currently updates it after undersampling + oversampling.
