...
srun -N1 -n1 --ntasks=1 --mem=8G --cpus-per-task=1 --time=03:00:00 \
  ...
  /bin/bash -lc "
    ...
    stdbuf -oL -eL ${VENV_DIR}/bin/flwr run . ailab --stream 2>&1 | tee /workspace/fl-backend/aiLab_scripts/logs/serverapp.log
  "
