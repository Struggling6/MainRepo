import os
os.environ["TORCH_BLAS_PREFER_HIPBLASLT"] = "0"

import threading
import optuna
import torch
import torch.nn as nn
import gc

from copy import deepcopy
from training.training_utils.TrainEvalBase import TrainEvalBase
from training.training_utils.utils import compute_pos_weight
from models.utils import get_device
from local_experiment import CONFIG, resolve_loss_fn
from optuna_config import OPTUNA_CONFIG, OptunaConfig
from pathlib import Path


class OptunaOptimizer(TrainEvalBase):
    """
    Runs an Optuna hyperparameter search using the shared train/val
    epoch logic from TrainEvalBase. Builds a fresh model for each trial.

    Search spaces and study settings are controlled by optuna_config.py.
    """

    optuna.logging.set_verbosity(optuna.logging.WARNING)
    _print_lock = threading.Lock()

    def _logger(self, msg: str):
        with self._print_lock:
            print(f"[Optuna] {msg}", flush=True)

    def __init__(
        self,
        X_train,
        y_train,
        X_val,
        y_val,
        config=CONFIG,
        optuna_config: OptunaConfig = OPTUNA_CONFIG,
        n_trials=None,
        epochs=None,
        storage=None,
        study_name=None,
        n_jobs=None,
    ): 
        self.optuna_config = optuna_config

        epochs = epochs if epochs is not None else optuna_config.general.epochs
        super().__init__(num_classes=1, epochs=epochs)

        self.config = config
        self.X_train = X_train
        self.y_train = y_train
        self.X_val = X_val
        self.y_val = y_val
        self.in_channels = int(X_train.shape[-1])
        self.config.evaluation.input_dim = self.in_channels

        self.n_trials = (
            n_trials if n_trials is not None else optuna_config.general.n_trials
        )
        self.study_name = (
            study_name if study_name is not None else optuna_config.general.study_name
        )
        if storage is not None:
            self.storage = storage
        else:
            _db_path = Path(__file__).resolve().parents[1] / "optuna_study.db"
            _db_path.parent.mkdir(parents=True, exist_ok=True)

            self.storage = f"sqlite:///{_db_path.as_posix()}"


        self.n_jobs = (
            n_jobs if n_jobs is not None else optuna_config.general.n_jobs
        )

        self.loss_fn = config.model.loss_fn
        self.raw_pw = compute_pos_weight(y_train)
        self.device = get_device()

    def _suggest_shared_params(self, trial, space):
        lr = trial.suggest_float(
            "lr",
            space.lr.low,
            space.lr.high,
            log=space.lr.log,
        )

        weight_decay = trial.suggest_float(
            "weight_decay",
            space.weight_decay.low,
            space.weight_decay.high,
            log=space.weight_decay.log,
        )

        num_layers = trial.suggest_int(
            "num_layers",
            space.num_layers.low,
            space.num_layers.high,
            log=space.num_layers.log,
        )

        batch_size = trial.suggest_categorical(
            "batch_size",
            space.batch_size,
        )

        pos_weight_cap = trial.suggest_float(
            "pos_weight_cap",
            space.pos_weight_cap.low,
            space.pos_weight_cap.high,
            log=space.pos_weight_cap.log,
        )

        dropout = trial.suggest_float(
            "dropout",
            space.dropout.low,
            space.dropout.high,
            log=space.dropout.log,
        )

        return lr, weight_decay, num_layers, batch_size, pos_weight_cap, dropout

    def _get_model_space(self):
        from config import (
            CNNTransformerConfig,
            TransformerConfig,
            LSTMConfig,
            PatchTSTConfig,
        )

        if isinstance(self.config.model, CNNTransformerConfig):
            return self.optuna_config.cnn_transformer

        if isinstance(self.config.model, TransformerConfig):
            return self.optuna_config.transformer

        if isinstance(self.config.model, LSTMConfig):
            return self.optuna_config.lstm

        if isinstance(self.config.model, PatchTSTConfig):
            return self.optuna_config.patchtst

        raise TypeError(
            f"No Optuna search space defined for {type(self.config.model).__name__}."
        )

    def run(self):
        self._logger(
            f"Starting study name={self.study_name}, trials={self.n_trials}, "
            f"epochs={self.epochs}, train_shape={self.X_train.shape}, "
            f"val_shape={self.X_val.shape}, device={self.device}"
        )

        study = optuna.create_study(
            study_name=self.study_name,
            storage=self.storage,
            direction="maximize",
            pruner=optuna.pruners.HyperbandPruner(
                min_resource=max(
                    3,
                    int(
                        self.epochs
                        * self.optuna_config.pruner.min_resource_fraction
                    ),
                ),
                max_resource=self.epochs,
                reduction_factor=self.optuna_config.pruner.reduction_factor,
            ),
            load_if_exists=True,
        )

        study.optimize(
            self._objective,
            n_trials=self.n_trials,
            n_jobs=self.n_jobs,
            callbacks=[self._pretty_trial_callback],
            gc_after_trial=True,
            show_progress_bar=True,
        )

        self._print_results(study)
        self._logger("Study finished")
        return study

    def _objective_score(self, pr_auc: float, f1: float) -> float:
        return (
            self.optuna_config.scoring.pr_auc_weight * float(pr_auc)
            + self.optuna_config.scoring.f1_weight * float(f1)
        )

    def _objective(self, trial):
        model = None
        optimizer = None
        train_dl = None
        val_dl = None

        try:
            space = self._get_model_space()

            (
                lr,
                weight_decay,
                num_layers,
                batch_size,
                pos_weight_cap,
                dropout,
            ) = self._suggest_shared_params(trial, space)

            self._logger(
                f"Trial {trial.number + 1}: sampled lr={lr:.3e}, "
                f"wd={weight_decay:.3e}, layers={num_layers}, "
                f"batch_size={batch_size}, dropout={dropout:.3f}, "
                f"pos_weight_cap={pos_weight_cap:.3f}"
            )

            model = self._build_model(trial, dropout, num_layers, batch_size)
            loss_fn = self._build_loss(pos_weight_cap)

            train_dl = self._build_dataloader(
                self.X_train,
                self.y_train,
                batch_size,
                shuffle=True,
            )

            val_dl = self._build_dataloader(
                self.X_val,
                self.y_val,
                batch_size,
                shuffle=False,
            )

            optimizer = torch.optim.AdamW(
                model.parameters(),
                lr=lr,
                weight_decay=weight_decay,
            )

            best_score = 0.0
            best_pr_auc = 0.0
            best_f1 = 0.0
            best_threshold = 0.5

            for epoch in range(self.epochs):
                train_loss = self._train_epoch(
                    model,
                    train_dl,
                    optimizer,
                    loss_fn,
                )

                val_loss, val_f1, best_thresh, pr_auc = self._val_epoch(
                    model,
                    val_dl,
                    loss_fn,
                )

                score = self._objective_score(pr_auc, val_f1)

                if score > best_score:
                    best_score = score
                    best_pr_auc = pr_auc
                    best_f1 = val_f1
                    best_threshold = best_thresh

                if self.optuna_config.scoring.report_metric == "score":
                    report_value = best_score
                elif self.optuna_config.scoring.report_metric == "pr_auc":
                    report_value = best_pr_auc
                elif self.optuna_config.scoring.report_metric == "f1":
                    report_value = best_f1
                else:
                    raise ValueError(
                        "optuna_config.scoring.report_metric must be "
                        "'score', 'pr_auc', or 'f1'"
                    )

                self._logger(
                    f"Trial {trial.number + 1} epoch {epoch + 1}/{self.epochs}: "
                    f"train_loss={train_loss:.6f}, val_loss={val_loss:.6f}, "
                    f"val_f1={val_f1:.4f}, pr_auc={pr_auc:.4f}, "
                    f"score={score:.4f}, best_score={best_score:.4f}, "
                    f"best_thresh={best_threshold:.2f}"
                )

                trial.report(report_value, epoch)

                if trial.should_prune():
                    self._logger(
                        f"Trial {trial.number + 1} pruned at epoch {epoch + 1} "
                        f"with report_value={report_value:.4f}"
                    )
                    raise optuna.TrialPruned()

            trial.set_user_attr("score", float(best_score))
            trial.set_user_attr("pr_auc", float(best_pr_auc))
            trial.set_user_attr("f1", float(best_f1))
            trial.set_user_attr("threshold", float(best_threshold))
            trial.set_user_attr("best_threshold", float(best_threshold))

            self._logger(
                f"Trial {trial.number + 1} complete: "
                f"score={best_score:.4f}, pr_auc={best_pr_auc:.4f}, "
                f"f1={best_f1:.4f}, threshold={best_threshold:.2f}"
            )

            return float(best_score)

        finally:
            del model, optimizer, train_dl, val_dl
            gc.collect()

            if torch.cuda.is_available():
                torch.cuda.empty_cache()

    def _build_model(self, trial, dropout, num_layers, batch_size) -> nn.Module:
        from config import (
            CNNTransformerConfig,
            TransformerConfig,
            LSTMConfig,
            PatchTSTConfig,
        )

        model_config = deepcopy(self.config.model)
        model_config.batch_size = batch_size
        model_config.num_layers = num_layers

        if isinstance(model_config, CNNTransformerConfig):
            space = self.optuna_config.cnn_transformer

            model_config.d_model = trial.suggest_categorical(
                "d_model",
                space.d_model,
            )

            model_config.dropout = dropout

            if hasattr(model_config, "nhead"):
                valid_nheads = [
                    n for n in space.nhead_candidates
                    if model_config.d_model % n == 0
                ]

                model_config.nhead = trial.suggest_categorical(
                    "nhead",
                    valid_nheads,
                )

                self._logger(
                    f"Trial {trial.number + 1}: valid_nheads={valid_nheads}, "
                    f"selected_nhead={model_config.nhead}"
                )

            self._logger(
                f"Trial {trial.number + 1}: building CNNTransformerConfig "
                f"d_model={model_config.d_model}, "
                f"num_layers={model_config.num_layers}, "
                f"dropout={model_config.dropout:.3f}"
            )

        elif isinstance(model_config, TransformerConfig):
            space = self.optuna_config.transformer

            model_config.d_model = trial.suggest_categorical(
                "d_model",
                space.d_model,
            )

            model_config.dropout = dropout

            if hasattr(model_config, "nhead"):
                valid_nheads = [
                    n for n in space.nhead_candidates
                    if model_config.d_model % n == 0
                ]

                model_config.nhead = trial.suggest_categorical(
                    "nhead",
                    valid_nheads,
                )

                self._logger(
                    f"Trial {trial.number + 1}: valid_nheads={valid_nheads}, "
                    f"selected_nhead={model_config.nhead}"
                )

            self._logger(
                f"Trial {trial.number + 1}: building TransformerConfig "
                f"d_model={model_config.d_model}, "
                f"num_layers={model_config.num_layers}, "
                f"dropout={model_config.dropout:.3f}"
            )

        elif isinstance(model_config, LSTMConfig):
            space = self.optuna_config.lstm

            model_config.hidden_size = trial.suggest_categorical(
                "hidden_size",
                space.hidden_size,
            )

            model_config.dropout = dropout

            self._logger(
                f"Trial {trial.number + 1}: building LSTMConfig "
                f"hidden_size={model_config.hidden_size}, "
                f"num_layers={model_config.num_layers}, "
                f"dropout={model_config.dropout:.3f}"
            )

        elif isinstance(model_config, PatchTSTConfig):
            space = self.optuna_config.patchtst

            patch_candidates = [
                p for p in space.patch_length_candidates
                if model_config.context_length % p == 0
            ]

            model_config.d_model = trial.suggest_categorical(
                "d_model",
                space.d_model,
            )

            model_config.patch_length = trial.suggest_categorical(
                "patch_length",
                patch_candidates,
            )

            model_config.patch_stride = trial.suggest_int(
                "patch_stride",
                1,
                model_config.patch_length,
            )

            model_config.ffn_dim = trial.suggest_categorical(
                "ffn_dim",
                space.ffn_dim,
            )

            model_config.channel_attention = trial.suggest_categorical(
                "channel_attention",
                space.channel_attention,
            )

            model_config.attention_dropout = trial.suggest_float(
                "attention_dropout",
                space.attention_dropout.low,
                space.attention_dropout.high,
                log=space.attention_dropout.log,
            )

            model_config.positional_dropout = trial.suggest_float(
                "positional_dropout",
                space.positional_dropout.low,
                space.positional_dropout.high,
                log=space.positional_dropout.log,
            )

            model_config.head_dropout = trial.suggest_float(
                "head_dropout",
                space.head_dropout.low,
                space.head_dropout.high,
                log=space.head_dropout.log,
            )

            self._logger(
                f"Trial {trial.number + 1}: building PatchTSTConfig "
                f"context_length={model_config.context_length}, "
                f"patch_candidates={patch_candidates}, "
                f"selected_patch_length={model_config.patch_length}, "
                f"patch_stride={model_config.patch_stride}, "
                f"d_model={model_config.d_model}, "
                f"ffn_dim={model_config.ffn_dim}"
            )

        else:
            raise TypeError(
                f"No search space defined for {type(model_config).__name__}. "
                "Add a branch to _build_model() in OptunaOptimizer."
            )

        return model_config.build(input_dim=self.in_channels).to(self.device)

    def _build_loss(self, pos_weight_cap):
        pw = min(self.raw_pw, pos_weight_cap)
        loss_cls = resolve_loss_fn(self.loss_fn)

        self._logger(
            f"Loss setup: loss_fn={self.loss_fn}, "
            f"raw_pos_weight={self.raw_pw:.4f}, "
            f"cap={pos_weight_cap:.4f}, "
            f"effective_pos_weight={pw:.4f}"
        )

        return loss_cls(pos_weight=torch.tensor([pw], device=self.device))

    def _print_results(self, study):
        print("\n=== Best Trial ===")
        print(f"  Score:      {study.best_trial.value:.4f}")
        print(f"  PR-AUC:     {study.best_trial.user_attrs.get('pr_auc')}")
        print(f"  F1:         {study.best_trial.user_attrs.get('f1')}")
        print(f"  Threshold:  {study.best_trial.user_attrs.get('threshold')}")
        print("  Params:")

        for k, v in study.best_trial.params.items():
            print(f"    {k}: {v}")

    def _pretty_trial_callback(
        self,
        study: optuna.Study,
        trial: optuna.trial.FrozenTrial,
    ):
        try:
            is_best = study.best_trial.number == trial.number
        except ValueError:
            is_best = False

        marker = "★ NEW BEST" if is_best else ""
        duration = trial.duration.total_seconds() if trial.duration else 0.0

        print(f"\n── Trial {trial.number + 1:>3}  {marker}")
        print(f"   value    : {trial.value}")
        print(f"   duration : {duration:6.1f}s")
        print("   metrics  :")
        print(f"     score           = {trial.user_attrs.get('score')}")
        print(f"     pr_auc          = {trial.user_attrs.get('pr_auc')}")
        print(f"     f1              = {trial.user_attrs.get('f1')}")
        print(f"     threshold       = {trial.user_attrs.get('threshold')}")
        print("   params   :")

        for k, v in trial.params.items():
            if isinstance(v, float):
                print(f"     {k:<16} = {v:.6g}")
            else:
                print(f"     {k:<16} = {v}")

        print("─" * 40 + "\n")